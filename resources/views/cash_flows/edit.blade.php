    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                {!! Form::open(['route' => ['cash-flows.update', $cashFlow->id], 'method' => 'POST']) !!}
                @csrf
                @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="cash_flow_types_id" id="cash_flow_type" class="form-control">
                            ...
                            @foreach ($cashFlowTypes as $cashFlowType)
                                <option value="{{ $cashFlowType->id }}"
                                    {{ $cashFlowType->id == $cashFlow->cash_flow_types_id ? 'selected' : '' }}>
                                    {{ $cashFlowType->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="payment_types_id" id="payment_type" class="form-control">
                            ...
                            @foreach ($paymentTypes as $paymentType)
                                <option value="{{ $paymentType->id }}"
                                    {{ $paymentType->id == $cashFlow->payment_types_id ? 'selected' : '' }} ">{{ $paymentType->name }}</option>
                                                     @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="date">Tanggal</label>
                        {!! Form::text('date', $cashFlow->date, ['placeholder' => '', 'class' => 'form-control', 'id' => 'date']) !!}
                    </div>
                    <div class="form-group">
                        <label for="amount">Jumlah nominal</label>
                        {!! Form::text('amount', $cashFlow->amount, ['placeholder' => '', 'class' => 'form-control']) !!}
                    </div>
                    <div class="form-group">
                        <label for="description">Deskripsi</label>
                        {!! Form::textarea('description', $cashFlow->description, ['placeholder' => 'Description', 'class' => 'form-control']) !!}
                    </div>
                </div>
                <!-- /.card-body -->
